...
test("requires worker dependencies to install from their committed lockfile", expectMutationFailure({
  mutateDockerfile: (dockerfile) => dockerfile.replace("RUN npm ci --omit=dev", "RUN npm install --omit=dev"),
}, /worker dependencies must install from their lockfile/));
