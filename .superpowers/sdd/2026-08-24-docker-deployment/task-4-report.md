...
- `redis` has no host port, is persistent and append-only, uses `noeviction`,
  and has a `redis-cli ping` healthcheck. The application network permits
  egress so external managed services remain reachable.
...
- `postgres` is a no-host-port `local-db` profile service; `seed` is a no-host-
  port `demo` profile service. External database/Redis URLs remain runtime
  values, while `NEXT_PUBLIC_APP_URL` is the sole build argument. The explicit
  `docker-compose.local-db.yml` overlay deterministically forces all database
  consumers to `postgres:5432`, even when `.env` has external URLs.
...
- `npm test` — 28 files, 88 tests passed.
- `npm run lint` — passed.
...
egress-capable application network, forced local-db URL overlay, and worker Redis
liveness command.
