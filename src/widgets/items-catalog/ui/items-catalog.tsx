...
type ItemsCatalogProps = { userId: string | null };

export function ItemsCatalog({ userId }: ItemsCatalogProps) {
  ...
  return (
    <section aria-labelledby="catalog-title">
      ...
      <div className="film-grid">{items.map((item) => <ItemCard key={item.id} item={item} action={userId ? <FavoriteButton item={item} userId={userId} /> : undefined} />)}</div>
    </section>
  );
}
